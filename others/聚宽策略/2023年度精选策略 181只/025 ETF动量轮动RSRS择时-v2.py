...
# 原文网址：https://www.joinquant.com/post/37480
# 标题：ETF动量轮动RSRS择时-v2
# 作者：Deemoo
...
# 趋势因子剧烈下跌时，往往这时候容易出现价格暴跌，
# 与个股的类似，为防止暴跌，因此基于动量因子的均线策略就出现了
# 若跌破均线，则卖出。
...
def initialize(context):
    ...
    set_order_cost(OrderCost(open_tax=0, close_tax=0, open_commission=0.0003, close_commission=0.0003, close_today_commission=0, min_commission=0),
                   type='fund')
    ...
    g.stock_pool = [
        ...
        '159949.XSHE', # 创业板500 
        # '159915.XSHE', # 创业指数，替代创业500进行18年前的回测
        # '510500.XSHG', # 500ETF
        # '159915.XSHE', # 创业板 ETF
        '159928.XSHE', # 中证消费ETF
        # '512120.XSHG', # 医药50ETF
...
def initial_slope_series():
    ...
    multe_data = [get_ols(data.low[i:i+g.N], data.high[i:i+g.N]) for i in range(g.M)]
    slopes = [i[1] for i in multe_data]
    return slopes

# 获取rsrs均线
def get_rsrs_score_mean(rsrs_score):
    N = 12
    g.rsrs_score_hisitory.append(rsrs_score)
    
    if(len(g.rsrs_score_hisitory)<N):
        return 0
    return mean(g.rsrs_score_hisitory[-N:])
    
# 只看RSRS因子值作为买入、持有和清仓依据，前版本还加入了移动均线的上行作为条件
def get_timing_signal(context,stock):
    
    data = attribute_history(g.ref_stock, g.N, '1d', ['high', 'low','volume'])
    intercept, slope, r2 = get_ols(data.low, data.high)
    # log.info('intercept={}, slope={}, r2={} '.format(intercept,slope,r2))
    g.slope_series.append(slope)
 
    rsrs_score = get_zscore(g.slope_series[-g.M:]) * r2
    log.info('rsrs_score {:.3f} '.format(rsrs_score))
    
    rsrs_mean = get_rsrs_score_mean(rsrs_score)
    
    record(rsrs_score=rsrs_score,rsrs_mean=rsrs_mean)
    
    if(rsrs_score>0 and rsrs_score < rsrs_mean):
        return "SELL"
