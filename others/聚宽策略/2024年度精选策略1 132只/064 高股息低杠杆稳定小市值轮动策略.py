...
# 原文网址：https://www.joinquant.com/post/39774
# 标题：高股息低杠杆小市值轮动策略
# 作者：wywy1995
...
def get_dividend_ratio_filter_list(context, stock_list, sort, p1, p2):
    time1 = context.previous_date    # 获取前一天的日期，用于确定查询的时间范围
    time0 = time1 - datetime.timedelta(days=365)   # 计算一年前的日期，用于确定查询的时间范围 
    # 获取分红数据，由于finance.run_query最多返回4000行，因此需要分批查询并组合结果
    interval = 1000  # 为了避免一次查询过多数据，设置一个合理的查询间隔
    ...
    # 限制查询的股票列表在interval内
    df = finance.run_query(q)  # 执行查询
    # 如果股票列表长度超过interval，则需要分批查询并拼接
    if list_len > interval:
        df_num = list_len // interval   # 计算需要查询的批次数量
        for i in range(df_num):
            ...
            temp_df = finance.run_query(q)  # 执行查询
            df = df.append(temp_df)         # 将查询结果追加到df中
    # 处理分红数据，填充缺失值为0，并设置股票代码为索引
    ...
    cap = get_fundamentals(q, date=time1)  # 获取市值数据
    cap = cap.set_index('code')            # 设置股票代码为索引
    #计算股息率
    DR = pd.concat([dividend, cap] ,axis=1, sort=False)    # 将分红数据和市值数据合并。
    DR['dividend_ratio'] = (DR['bonus_amount_rmb']/10000) / DR['market_cap']  # 计算股息率
    # 根据股息率进行排序并筛选
    DR = DR.sort_values(by=['dividend_ratio'], ascending=sort)  # 根据股息率进行排序。
    final_list = list(DR.index)[int(p1*len(DR)):int(p2*len(DR))]  # 根据比例p1和p2筛选股票列表。
    return final_list# 返回筛选后的股票列表

# 1-2 定义选股模块的函数，用于筛选出符合特定条件的股票列表
def get_stock_list(context):
    yesterday = context.previous_date   # 获取前一天的日期，用于确定数据查询的时间范围
    initial_list = get_all_securities().index.tolist() # 获取所有上市股票的代码列表
    initial_list = filter_kcbj_stock(initial_list) # 过滤掉科创板及创业板股票
    initial_list = filter_new_stock(context, initial_list, 375) # 过滤掉新上市的股票，这里假设上市天数小于375天为新上市股票
    initial_list = filter_st_stock(initial_list) # 过滤掉ST股票
    # 获取高股息率的股票列表，高股息(全市场最大25%)
    dr_list = get_dividend_ratio_filter_list(context, initial_list, False, 0, 0.5) # 股息率最高的前50%股票会被选中，False表示降序排序
    # 在高股息率股票列表中，进一步筛选高波动性的股票
    tv_list = get_factor_filter_list(context, dr_list, 'turnover_volatility', False, 0, 0.8) # 'turnover_volatility'是用于衡量股票波动性的因子，False表示降序排序，取前80%的股票
    # 在高波动性股票列表中，进一步筛选低负债率的股票
    lev_list = get_factor_filter_list(context, tv_list, 'MLEV', True, 0, 0.5) # 'MLEV'是用于衡量股票负债率的因子，True表示升序排序，取后50%的股票
    # 对低负债率的股票进行流通市值排序，选取流通市值最小的前15只股票
...
def get_factor_filter_list(context, stock_list, jqfactor, sort, p1, p2):  # 定义一个函数，用于根据特定因子对股票进行筛选
    yesterday = context.previous_date  # 获取前一天的日期
    # 获取特定因子的值
    score_list = get_factor_values(stock_list, jqfactor, end_date=yesterday, count=1)[jqfactor].iloc[0].tolist()
    # 创建一个DataFrame，用于存储股票代码和对应的因子得分
    df = pd.DataFrame(columns=['code','score'])
    df['code'] = stock_list
    df['score'] = score_list
    df = df.dropna()    # 去除缺失值
    df.sort_values(by='score', ascending=sort, inplace=True)   # 根据因子得分进行排序
    filter_list = list(df.code)[int(p1*len(df)):int(p2*len(df))]  # 根据给定的比例p1和p2筛选股票
