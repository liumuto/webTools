...
# 原文网址：https://www.joinquant.com/post/44491
...
def linreg(x,y):
    x = sm.add_constant(array(x))    # 为自变量 x 添加常数项，这是因为 OLS 回归模型需要一个截距项，sm.add_constant 是 statsmodels 库中的函数，用于添加常数项
    y = array(y)  # 将因变量 y 转换为 numpy 数组
    # 检查因变量 y 的长度是否大于 1，以确保有足够的数据进行回归分析
    if len(y)>1:
        # 创建 OLS 回归模型并拟合数据
        # regression.linear_model.OLS 是 statsmodels 库中的函数，用于创建普通最小二乘回归模型
        model = regression.linear_model.OLS(y, x).fit()
        # 从拟合的模型中提取 beta 值，即斜率参数
        # model.params[1] 表示模型参数的第二个值，第一个值是截距项，第二个值是斜率
        x = model.params[1]
...
def fisher_transform_strategy(context, security_list):
    ...
    # 设置周期长度，用于计算 Fisher Transform
    period = 10
    for security in security_list:
        # 获取每只股票过去 'period' 天的价格数据
        ...
        def fisher_transform(data, period=period):
            ...
            # 计算 Fisher Transform 结果
            fft_result = np.fft.fft(smoothed_price)
            fisher = np.log((1 + abs_diff) / (1 - abs_diff))

            return fisher

        # 计算 Fisher Transform 值，并将其添加到 DataFrame 中
        df['fisher'] = fisher_transform(df)

        # 生成择时信号，Fisher Transform 值大于零则为买入信号（1），否则为卖出信号（-1）
        df['signal'] = np.where(df['fisher'] > 0, 1, -1)

        # 判断最后一条 Fisher Transform 值是否大于零，如果是，则将该股票加入可买入股票列表
        if df['fisher'].iloc[-1] > 0:
            buyable_stocks.append(security)

    return buyable_stocks   # 返回可买入股票列表
