...
def weekly_adjustment(context):
    #获取应买入列表
    target_list = get_stock_list(context)     # 获取应买入的股票列表
    target_list = filter_paused_stock(target_list)    # 过滤掉停牌的股票
    target_list = filter_limitup_stock(context, target_list)    # 过滤掉涨停的股票
    target_list = filter_limitdown_stock(context, target_list)    # 过滤掉跌停的股票
    #过滤最近买过且涨停过的股票
    recent_limit_up_list = get_recent_limit_up_stock(context, target_list, g.limit_days)
    # 获取黑名单股票列表，即最近买过且涨停过的股票
    ...
    for stock in g.hold_list:
        if (stock not in target_list) and (stock not in g.high_limit_list):      # 如果股票不在目标列表中且不是涨停股票，则卖出
            log.info("卖出[%s]" % (stock))  # 如果股票不在目标列表中且不是涨停股票，则记录卖出信息
            position = context.portfolio.positions[stock]  # 获取当前股票的持仓信息
            close_position(position)  # 调用close_position函数卖出该股票，清空该股票的持仓
        else:
            log.info("已持有[%s]" % (stock))  # 如果股票在目标列表中或为涨停股票，则记录已持有信息
    # 调仓买入：计算需要买入的股票数量
    position_count = len(context.portfolio.positions)  # 计算当前投资组合中的持仓数量
    target_num = len(target_list)  # 计算目标股票列表中的股票数量
    if target_num > position_count:    # 如果目标股票数量大于当前持仓数量，表示需要买入新的股票
        value = context.portfolio.cash / (target_num - position_count)  # 计算每只股票的买入金额，将剩余现金平均分配到每一只未持仓的股票上
        for stock in target_list:  # 遍历目标股票列表
            if context.portfolio.positions[stock].total_amount == 0:  # 检查当前投资组合是否已经持有该股票，如果没有持有，则尝试买入
                if open_position(stock, value):  # 调用open_position函数尝试买入股票，传入股票代码和计划买入的金额
                    if len(context.portfolio.positions) == target_num:  # 如果买入成功，并且当前持仓数量达到了目标持仓数量，则停止进一步买入。
                        break
