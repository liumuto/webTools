...
# 原文网址：https://www.joinquant.com/post/44487
...
def get_stocks(context):
    # 过滤次新股，上市超过 180 天的股票才会被选中
    by_date = context.previous_date - datetime.timedelta(days=180)
    # 使用 get_all_securities 函数获取在 by_date 指定日期上市的所有证券
    # date 参数传入 by_date 来过滤得到特定日期的证券信息
    # index 属性返回一个包含所有证券代码的 Index 对象
    # tolist() 方法将 Index 对象转换为一个 Python 列表
    stocks  = get_all_securities(date=by_date).index.tolist()
    # 对股票进行全面过滤
    stocks = filter_all_stocks(context,stocks)
    # 计算筛选后股票的股息率，高股息(全市场最大25%)
