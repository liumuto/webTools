...
## 2025-01-XX 平均成本体系买卖信号与拉升前选股策略

### 问题描述
用户要求根据平均成本分析文档，编写通达信的平均成本体系买入卖出信号逻辑，并提供一个走势拉升前的策略选股信号。

### 解决方案与思路

基于 `平均成本分析.md` 文档中的理论，构建完整的平均成本体系交易系统：

1. **平均成本计算**：使用 `COST(50)` 函数计算50%获利盘成本，代表市场平均持仓成本
2. **筹码集中度分析**：通过 `COST(95)-COST(5)` 计算筹码分布区间，判断筹码集中程度
3. **斜率分析**：计算平均成本的斜率变化，判断成本趋势
4. **量价配合**：结合成交量特征，判断主力行为
5. **多信号组合**：综合多个条件，提高信号可靠性

### 主要代码变更

#### 1. 主图指标：平均成本体系买卖信号.tdx
...
**买入信号系统**：
- **买入信号1**：洗盘后稳定 - 平均成本稳定+筹码集中+缩量+价格接近成本
- **买入信号2**：拉升启动 - 平均成本上拐+筹码集中+温和放量+多头排列
- **买入信号3**：主升浪确认 - 平均成本上升+筹码高度集中+放量+价格上涨
- **底部拐点**：平均成本斜率转正+筹码集中+量能变化

**卖出信号系统**：
- **卖出信号1**：派发阶段 - 平均成本上升+筹码分散+暴量+价格偏离
- **卖出信号2**：出货完成 - 平均成本横盘+筹码分散+价格跌破成本
- **卖出信号3**：下跌确认 - 平均成本下降+筹码分散+价格破位
- **顶背离**：平均成本上升但价格下跌+筹码分散

#### 2. 选股公式：平均成本拉升前选股策略.tdx

**核心选股逻辑**：
- **条件1**：平均成本下降后趋稳或上拐（主力吸筹完成）
- **条件2**：筹码集中度上升或维持高位（主力控盘）
- **条件3**：成交量特征（缩量整理或温和放量）
- **条件4**：价格在平均成本附近（有上涨空间）
- **条件5**：技术指标配合（均线、KDJ、MACD）
- **条件6**：价格突破或接近突破
- **条件7**：价格与平均成本关系良好
- **条件8**：排除ST股和跌停股
...
1. **平均成本计算**：
   - 使用 `COST(50)` 计算50%获利盘成本
   - 通过移动平均平滑处理

2. **筹码集中度**：
   - 计算公式：`(COST(95)-COST(5))/((COST(95)+COST(5))/2)`
   - 数值越小表示筹码越集中

3. **斜率分析**：
   - 使用差分法计算斜率：`(当前值-前N日值)/N`
   - 判断斜率转正、上升、下降等状态
...
**问题1**：通达信公式中筹码集中度的计算
- **解决**：使用 `COST(95)-COST(5)` 计算筹码分布区间，除以平均价格得到相对集中度

**问题2**：斜率计算的稳定性
- **解决**：使用差分法计算斜率，并通过移动平均平滑处理

**问题3**：选股条件的平衡
- **解决**：采用核心条件+辅助条件的组合方式，既保证质量又不过于严格

### 后续优化建议

1. **参数优化**：
   - 对斜率周期、筹码集中度阈值等参数进行回测优化
   - 针对不同市场环境调整参数
...
1. `src/stocks/docs/平均成本体系买卖信号.tdx` - 主图指标公式
2. `src/stocks/docs/平均成本拉升前选股策略.tdx` - 选股公式
3. `src/stocks/docs/平均成本体系使用说明.md` - 使用说明文档

### 使用建议

1. **主图指标**：在K线图上加载，用于判断买卖时机
2. **选股公式**：定期运行选股，筛选符合条件的股票
3. **结合使用**：选股后结合主图指标进行技术分析
4. **风险控制**：设置止损位，控制仓位，不要满仓操作
5. **多周期验证**：在日线、周线等多个周期验证信号
